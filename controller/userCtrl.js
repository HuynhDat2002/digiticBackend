...
const getMyOrders = asyncHandler(async (req, res) => {
  const { _id } = req.user;
  try {
    const orders = await Order.find({ user: _id }).populate("user").populate("orderItems.productId").populate("orderItems.color")
    res.json({
      orders
    })
  }
...
const {id}=req.params
  ...
  try {
    const orders = await Order.findOne({_id:id}).populate("user").populate("orderItems.productId").populate("orderItems.color")
    res.json({
      orders
    })
  }
